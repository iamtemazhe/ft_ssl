...
static t_data			*sha2_out(t_sha2_64 *sha2)
{
	if (!sha2->op.data)
...
static void				sha2_const_init(t_sha2_64 *sha2)
{
	int16_t				i;
	...
	sha2->c_k = g_sha2_k_64;
}

static t_bool			*sha2_init(t_sha2_64 *sha2, const void *data, size_t n)
{
	size_t				bits_len;

	bits_len = (n + 1) * BYTE_SIZE;
	while ((bits_len % 1024) != 896)
		bits_len += BYTE_SIZE;
	sha2->op.len = bits_len / BYTE_SIZE + SHA2_BLOCK_64 / BYTE_SIZE;
...
static t_data			*get_hash(t_sha2_64 *sha2)
{
	int16_t				i;
	uint8_t				*h;
	...
	i = BYTE_SIZE;
	while (i--)
		sha2->h_[i] = ft_swapint64(sha2->h_[i]);
	h = (uint8_t *)sha2->h_;
...
t_data					*sha2_64(t_sha2_64 *sha2, const void *data, size_t n)
{
	if (!data)
		return (NULL);
